...
import { ArrowLeft, Camera, MapPin, Check, QrCode, Share2, ExternalLink, Navigation } from "lucide-react";
import { getPosition, takePhoto, shareResult, haptic, notify, distanceMeters } from "@/lib/native";

type Phase = "approach" | "verify" | "shotmatch" | "complete";

export default function CaptureFlow({ sceneId }: { sceneId: string }) {
  ...
  async function useRealLocation() {
    if (!scene) return;
    setLocating(true);
    haptic("light");
    ...
    setLocating(false);
    if (!pos) {
      // Permission denied or unavailable — keep current simulated distance.
      return;
    }
    const meters = Math.round(distanceMeters(pos, { lat: scene.lat, lng: scene.lng }));
    setDistance(meters);
    if (meters <= 25) {
      haptic("medium");
      setPhase("verify");
  ...
  async function useNativeCamera() {
    ...
    if (dataUrl) {
      ...
      // No native plugin / permission — fall back to file picker
  ...
  async function onShare() {
    ...
    haptic("light");
    await shareResult({
      title: `${scene.filmTitle} — SetLocate GO`,
      text: `Captured ${scene.filmTitle} (${scene.year}) at ${scene.city} for ${scene.points} pts. From screen to street. 📍`,
          ...
          <motion.div key="approach" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
            <Card className="p-6 relative overflow-hidden">
              ...
              <div className="flex flex-wrap gap-2">
                <Button onClick={useRealLocation} variant="ghost" disabled={locating}>
                  <Navigation size={15} /> {locating ? "Locating…" : "Use my GPS"}
                </Button>
                <Button onClick={() => { haptic("light"); setDistance((d) => Math.max(0, d - 70)); }} variant="ghost">
                  Walk closer
                </Button>
                <Button onClick={() => { haptic("medium"); setDistance(12); setPhase("verify"); }}>
                  Simulate arrival
                </Button>
              </div>
              <p className="mono-meta text-ink-secondary mt-3 italic">
                GPS REAL ON DEVICE · SIMULATED IN DEV BUILDS.
              </p>
            </Card>
          </motion.div>
        )}
        ...
        {phase === "shotmatch" && (
          <motion.div key="shotmatch" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
            <Card className="p-6">
              ...
              <div className="flex flex-wrap gap-2">
                <Button onClick={useNativeCamera} variant="ghost">
                  <Camera size={16} /> {userImage ? "Retake" : "Take photo"}
                </Button>
                <Button
                  variant="outline"
                  onClick={() => { haptic("light"); setUserImage(scene.imageUrl); }}
                >
                  Use demo shot
                </Button>
                <Button onClick={() => { haptic("heavy"); setPhase("complete"); }} disabled={!userImage}>
                  Submit capture
                </Button>
              </div>
              <p className="mono-meta text-ink-secondary mt-3 italic">
            ...
            <Card className="p-8 text-center grain relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <div className="mt-7 flex flex-wrap items-center justify-center gap-2">
                  ...
                  <Button variant="outline" onClick={() => { haptic("light"); router.push("/map"); }}>
